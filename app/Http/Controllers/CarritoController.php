    ...
    public function realizarPedido(Request $request)
{
    ...
    $Usuario = Auth::user();
    Mail::to(Auth::user()->email)->send(new FacturaMailable($pedido, $Usuario, $detallesCarrito));
    ...
    foreach ($detallesCarrito as $detalle) 
    {
        ...
        $producto->cant_disponible -= $detalle->Cantidad_producto;
        $producto->save();
    }
    ...
    return redirect()->route('pedido.detalle', ['id' => $pedido->id])->with('success', 'El pedido se ha realizado correctamente.');
}
